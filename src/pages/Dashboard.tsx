...
export default function Dashboard() {
  ...
  const rate    = sample.length ? Math.round((sample.filter((l) => l.status === "success").length / sample.length) * 100) + "%" : "â€”";
  ...
  const handleSend = async () => {
    ...
    if (!isValidPhone(phone)) { showToast("Invalid phone number. Use +254... or 07..."); return; }

    // Simulator mode â€” no backend needed
    if (!s.gatewayUrl || !s.gatewayKey) {
      ...
      showToast("Sent to simulator!"); setTo(""); setMsg(""); setLogs(getLogs()); return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast("Message sent!"); setTo(""); setMsg(""); setLogs(getLogs());
      } else {
        addLog({ to: phone, message: msg, status: "failed" });
        showToast("Failed: " + (res.error || "Unknown")); setLogs(getLogs());
      }
      ...
      <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
            ...
            {!s.gatewayKey && <span style={{ fontSize: "0.75rem", color: "#C98B4A" }}>âš¡ Simulator mode â€” no backend needed</span>}
          </div>
        </div>
        ...
        <div className="card">
                ...
                <tbody>
                  {logs.slice(0, 8).map((l) => (
                    <tr key={l.id} style={{ borderBottom: "1px solid #F5F0EB" }}>
                      ...
                      <td style={{ padding: "10px 8px", color: "#C4AFA3", fontSize: "0.7rem" }}>{relTime(l.time)}</td>
                      <td style={{ padding: "10px 0", color: "#C98B4A", fontSize: "0.7rem" }}>{l.status === "success" ? (l.cost || "KES 0.80") : "â€”"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {logs.length > 8 && <div style={{ marginTop: 12, textAlign: "center" }}><Link to="/logs" style={{ fontSize: "0.75rem", color: "#C98B4A", textDecoration: "none" }}>View all {logs.length} logs â†’</Link></div>}
        </div>
      </div>
    </>
  );
