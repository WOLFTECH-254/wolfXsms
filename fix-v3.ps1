# ============================================================
#  wolfXsms — Fix simulator stability + hamburger icon
#  Run inside your wolfXsms folder:
#  .\fix-v3.ps1
# ============================================================
...
# ── Sidebar — use correct hamburger icon ─────────────────────
@'
...
Write-Host "  [OK] Sidebar.tsx — three-line hamburger" -ForegroundColor Green

# ── Simulator — stable layout, no bouncing ───────────────────
@'
...
export default function Simulator() {
  ...
  // Poll localStorage every 1.5s for new messages from other pages
  useEffect(() => {
    const id = setInterval(() => setMessages(getSimMessages()), 1500);
    return () => clearInterval(id);
  }, []);

  // Scroll phone to bottom when new messages arrive
  useEffect(() => {
  ...
  const filtered = filter.trim()
    ? messages.filter((m) =>
        m.to.includes(filter.trim()) ||
        m.to === formatPhone(filter.trim())
      )
    : messages;
  ...
  return (
    <>
      ...
      {/* Outer wrapper — fixed height so nothing bounces */}
      <div style={{ padding: "1.25rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={{
          ...
          <span>Messages sent from Dashboard or Send SMS appear here. No real SMS sent — free testing without a backend.</span>
        </div>

        {/* Two column grid — phone + form side by side */}
        <div style={{
          display: "grid",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div className="card">
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                {/* Fixed height textarea — this was causing the bounce */}
                <textarea
                  className="form-input"
          ...
          <div style={{ display: "flex", justifyContent: "center" }}>
            <div style={{
              ...
              /* Fixed height so phone never resizes */
              height: 520,
              display: "flex",
              ...
              <div style={{
                ...
                display: "flex",
                flexDirection: "column",
                /* Critical: prevent phone from growing */
                minHeight: 0,
              }}>
                {/* Status bar */}
                <div style={{
                ...
                {filtered.length === 0 ? (
                  <div style={{
                    ...
                    <span style={{ fontSize: 11, color: "#bbb" }}>
                      Send from the form or Dashboard
                    </span>
                  </div>
                ) : (
                  <div
                    ...
                    style={{
                      ...
                      gap: 8,
                      background: "#e8e8e8",
                      /* Prevents the phone from stretching */
                      minHeight: 0,
                    }}
                  >
...
'@ | Set-Content -Path "src\pages\Simulator.tsx" -Encoding UTF8
Write-Host "  [OK] Simulator.tsx — stable layout" -ForegroundColor Green
